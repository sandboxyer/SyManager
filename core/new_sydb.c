...
void generate_universally_unique_identifier(char* universally_unique_identifier) {
    ...
    for (int segment_index = 0; segment_index < 5; segment_index++) {
        if (segment_index > 0) universally_unique_identifier[current_position++] = '-';
        for (int character_index = 0; character_index < segment_lengths[segment_index]; character_index++) {
            universally_unique_identifier[current_position++] = hexadecimal_characters[rand_r(&random_seed) % 16];
        }
    }
...
int create_directory_recursively(const char* path) {
    ...
    if (stat(path, &status_info) == -1) {
        ...
        snprintf(temporary_path, sizeof(temporary_path), "%s", path);
        path_length = strlen(temporary_path);
        if (temporary_path[path_length - 1] == '/') {
            temporary_path[path_length - 1] = 0;
        }
        
        for (path_pointer = temporary_path + 1; *path_pointer; path_pointer++) {
            if (*path_pointer == '/') {
                *path_pointer = 0;
                if (mkdir(temporary_path, 0755) == -1 && errno != EEXIST) {
...
char* get_sydb_base_directory_path() {
...
void release_exclusive_lock(int file_descriptor, const char* lock_file_path) {
    if (file_descriptor != -1) {
...
void destroy_lru_cache(lru_cache_t* cache) {
    ...
    while (current_entry) {
        ...
        if (current_entry->instance) {
            if (current_entry->instance->binary_data) {
                free(current_entry->instance->binary_data);
            }
            free(current_entry->instance);
        }
        free(current_entry);
...
void lru_cache_put(lru_cache_t* cache, const char* universally_unique_identifier, database_instance_t* instance) {
    ...
    while (existing_entry) {
        if (strcmp(existing_entry->universally_unique_identifier, universally_unique_identifier) == 0) {
            if (existing_entry->instance->binary_data) {
                free(existing_entry->instance->binary_data);
            }
            ...
            if (existing_entry != cache->head_entry) {
                if (existing_entry->previous_entry) existing_entry->previous_entry->next_entry = existing_entry->next_entry;
                if (existing_entry->next_entry) existing_entry->next_entry->previous_entry = existing_entry->previous_entry;
                if (existing_entry == cache->tail_entry) cache->tail_entry = existing_entry->previous_entry;
                
                existing_entry->next_entry = cache->head_entry;
                existing_entry->previous_entry = NULL;
                if (cache->head_entry) cache->head_entry->previous_entry = existing_entry;
                cache->head_entry = existing_entry;
                if (!cache->tail_entry) cache->tail_entry = existing_entry;
            }
    ...
    cache_entry_t* new_cache_entry = malloc(sizeof(cache_entry_t));
    if (!new_cache_entry) {
        pthread_rwlock_unlock(&cache->lock);
        return;
    }
    ...
    if (cache->size > cache->capacity) {
        ...
        if (least_recently_used_entry) {
            ...
            if (least_recently_used_entry->instance) {
                if (least_recently_used_entry->instance->binary_data) {
                    free(least_recently_used_entry->instance->binary_data);
                }
                free(least_recently_used_entry->instance);
            }
            free(least_recently_used_entry);
            cache->size--;
        }
    }
...
database_instance_t* lru_cache_get(lru_cache_t* cache, const char* universally_unique_identifier) {
    ...
    while (cache_entry) {
        if (strcmp(cache_entry->universally_unique_identifier, universally_unique_identifier) == 0) {
            ...
            if (cache_entry != cache->head_entry) {
                if (cache_entry->previous_entry) cache_entry->previous_entry->next_entry = cache_entry->next_entry;
                if (cache_entry->next_entry) cache_entry->next_entry->previous_entry = cache_entry->previous_entry;
                if (cache_entry == cache->tail_entry) cache->tail_entry = cache_entry->previous_entry;
                
                cache_entry->next_entry = cache->head_entry;
                cache_entry->previous_entry = NULL;
                if (cache->head_entry) cache->head_entry->previous_entry = cache_entry;
                cache->head_entry = cache_entry;
                if (!cache->tail_entry) cache->tail_entry = cache_entry;
            }
...
b_tree_node_t* create_b_tree_node(bool is_leaf_node) {
    b_tree_node_t* new_node = malloc(sizeof(b_tree_node_t));
...
int b_tree_search_node(b_tree_node_t* node, const char* search_key, uint64_t* record_offset) {
    int key_index = 0;
...
void b_tree_insert_non_full_node(b_tree_node_t* node, const char* key, uint64_t record_offset) {
    ...
    if (node->is_leaf) {
        ...
        strcpy(node->keys[key_index + 1], key);
...
void b_tree_insert_into_index(field_index_t* index, const char* key, uint64_t record_offset) {
    pthread_rwlock_wrlock(&index->lock);
    
    b_tree_node_t* root_node = index->root_node;
    
    if (root_node->key_count == B_TREE_ORDER - 1) {
        b_tree_node_t* new_root_node = create_b_tree_node(false);
        new_root_node->child_node_offsets[0] = (uint64_t)root_node;
        index->root_node = new_root_node;
    } else {
        b_tree_insert_non_full_node(root_node, key, record_offset);
    }
...
FILE* open_data_file_with_optimizations(const char* database_name, const char* collection_name, const char* mode) {
    char file_path[MAXIMUM_PATH_LENGTH];
    snprintf(file_path, MAXIMUM_PATH_LENGTH, "%s/%s/%s/data%s", 
             get_sydb_base_directory_path(), database_name, collection_name, DATA_FILE_EXTENSION);
...
int initialize_high_performance_data_file(FILE* data_file) {
...
int read_file_header_information(FILE* data_file, file_header_t* file_header) {
    if (fseek(data_file, 0, SEEK_SET) != 0) return -1;
...
int write_file_header_information(FILE* data_file, file_header_t* file_header) {
    if (fseek(data_file, 0, SEEK_SET) != 0) return -1;
    if (fwrite(file_header, sizeof(file_header_t), 1, data_file) != 1) return -1;
    return fflush(data_file);
}
...
void initialize_collection_locks(collection_lock_t* locks) {
    pthread_rwlock_init(&locks->schema_lock, NULL);
    pthread_rwlock_init(&locks->data_lock, NULL);
    pthread_mutex_init(&locks->cache_lock, NULL);
    pthread_rwlock_init(&locks->index_lock, NULL);
    pthread_cond_init(&locks->write_complete_condition, NULL);
    locks->active_readers_count = 0;
    locks->waiting_writers_count = 0;
    locks->writer_active = false;
}

void acquire_collection_read_lock(collection_lock_t* locks) {
...
void release_collection_read_lock(collection_lock_t* locks) {
    pthread_mutex_lock(&locks->cache_lock);
    locks->active_readers_count--;
...
void acquire_collection_write_lock(collection_lock_t* locks) {
    pthread_mutex_lock(&locks->cache_lock);
...
void release_collection_write_lock(collection_lock_t* locks) {
    pthread_mutex_lock(&locks->cache_lock);
    locks->writer_active = false;
...
field_type_t parse_field_type_from_string(const char* type_string) {
...
const char* convert_field_type_to_string(field_type_t type) {
    switch (type) {
...
int parse_schema_fields_from_arguments(int argument_count, char* argument_values[], int start_index, 
    ...
    *field_count = 0;
    ...
    for (int argument_index = start_index; argument_index < argument_count && *field_count < MAXIMUM_FIELDS; argument_index++) {
        ...
        *first_dash = '\0';
        ...
        field_type_t type = parse_field_type_from_string(type_string);
...
int load_schema_from_file(const char* database_name, const char* collection_name, 
                         field_schema_t* fields, int* field_count) {
    char schema_file_path[MAXIMUM_PATH_LENGTH];
    snprintf(schema_file_path, MAXIMUM_PATH_LENGTH, "%s/%s/%s/schema.txt", 
             get_sydb_base_directory_path(), database_name, collection_name);
    ...
    *field_count = 0;
    ...
    while (fgets(line_buffer, sizeof(line_buffer), schema_file) && *field_count < MAXIMUM_FIELDS) {
        ...
        *second_colon = '\0';
        ...
        strncpy(fields[*field_count].name, field_name, MAXIMUM_FIELD_LENGTH - 1);
        fields[*field_count].name[MAXIMUM_FIELD_LENGTH - 1] = '\0';
        fields[*field_count].type = parse_field_type_from_string(type_string);
...
bool validate_field_value_against_schema(const char* field_name, const char* value, field_type_t type) {
...
int validate_instance_against_schema(const char* instance_json, 
    ...
    for (int field_index = 0; field_index < field_count; field_index++) {
        ...
        if (json_has_field(instance_json, fields[field_index].name)) {
            ...
            if (field_value) {
                if (!validate_field_value_against_schema(fields[field_index].name, field_value, fields[field_index].type)) {
                    free(field_value);
                    return -1;
                }
                free(field_value);
...
void print_collection_schema(const char* database_name, const char* collection_name) {
    field_schema_t fields[MAXIMUM_FIELDS];
    int field_count = 0;
    
    if (load_schema_from_file(database_name, collection_name, fields, &field_count) == -1) {
        fprintf(stderr, "Error: Cannot load schema for collection '%s'\n", collection_name);
        return;
    }
    ...
    for (int field_index = 0; field_index < field_count; field_index++) {
        printf("%-20s %-10s %-10s %-10s\n", 
               fields[field_index].name, 
               convert_field_type_to_string(fields[field_index].type),
...
int database_exists(const char* database_name) {
    ...
    snprintf(database_path, MAXIMUM_PATH_LENGTH, "%s/%s", 
             get_sydb_base_directory_path(), database_name);
...
int collection_exists(const char* database_name, const char* collection_name) {
    ...
    snprintf(collection_path, MAXIMUM_PATH_LENGTH, "%s/%s/%s", 
             get_sydb_base_directory_path(), database_name, collection_name);
...
int create_database(const char* database_name) {
    ...
    strncpy(base_directory, get_sydb_base_directory_path(), MAXIMUM_PATH_LENGTH - 1);
    base_directory[MAXIMUM_PATH_LENGTH - 1] = '\0';
    
    if (create_directory_recursively(base_directory) == -1) {
        return -1;
    }
    
    char database_path[MAXIMUM_PATH_LENGTH];
    snprintf(database_path, MAXIMUM_PATH_LENGTH, "%s/%s", base_directory, database_name);
    
    if (create_directory_recursively(database_path) == -1) {
        return -1;
    }
...
char** list_all_databases(int* database_count) {
    char base_directory[MAXIMUM_PATH_LENGTH];
    strncpy(base_directory, get_sydb_base_directory_path(), MAXIMUM_PATH_LENGTH - 1);
    base_directory[MAXIMUM_PATH_LENGTH - 1] = '\0';
...
int create_collection(const char* database_name, const char* collection_name, 
    ...
    char collection_path[MAXIMUM_PATH_LENGTH];
    snprintf(collection_path, MAXIMUM_PATH_LENGTH, "%s/%s", database_path, collection_name);
    
    if (create_directory_recursively(collection_path) == -1) {
        return -1;
    }
    
    char schema_file_path[MAXIMUM_PATH_LENGTH];
    snprintf(schema_file_path, MAXIMUM_PATH_LENGTH, "%s/schema.txt", collection_path);
    
    char lock_file_path[MAXIMUM_PATH_LENGTH];
    snprintf(lock_file_path, MAXIMUM_PATH_LENGTH, "%s/.schema.lock", collection_path);
    ...
    snprintf(data_file_path, MAXIMUM_PATH_LENGTH, "%s/data%s", collection_path, DATA_FILE_EXTENSION);
    FILE* data_file = fopen(data_file_path, "w+b");
    if (data_file) {
        initialize_high_performance_data_file(data_file);
        fclose(data_file);
    }
...
char** list_collections_in_database(const char* database_name, int* collection_count) {
    char database_path[MAXIMUM_PATH_LENGTH];
    snprintf(database_path, MAXIMUM_PATH_LENGTH, "%s/%s", 
             get_sydb_base_directory_path(), database_name);
...
// ==================== HIGH-PERFORMANCE INSTANCE OPERATIONS ====================

char* build_instance_json_from_fields_and_values(char** field_names, char** field_values, int field_count) {
    ...
    for (int field_index = 0; field_index < field_count; field_index++) {
        if (field_index > 0) strcat(json_string, ",");
        ...
        if ((field_values[field_index][0] == '[' && field_values[field_index][strlen(field_values[field_index])-1] == ']') ||
            ...
            snprintf(json_string + strlen(json_string), MAXIMUM_LINE_LENGTH - strlen(json_string), 
...
int insert_instance_into_collection(const char* database_name, const char* collection_name, char* instance_json) {
    ...
    char lock_file_path[MAXIMUM_PATH_LENGTH];
    snprintf(lock_file_path, MAXIMUM_PATH_LENGTH, "%s/.data.lock", collection_path);
    int lock_file_descriptor = acquire_exclusive_lock(lock_file_path);
    if (lock_file_descriptor == -1) {
        return -1;
    }
    ...
    snprintf(complete_json, sizeof(complete_json), "{\"_id\":\"%s\",\"_created_at\":%ld,%s", 
             universally_unique_identifier, time(NULL), instance_json + 1);
    
    FILE* data_file = open_data_file_with_optimizations(database_name, collection_name, "r+b");
    if (!data_file) {
    ...
    file_header_t file_header;
    if (read_file_header_information(data_file, &file_header) == -1) {
        if (initialize_high_performance_data_file(data_file) == -1) {
            fclose(data_file);
            release_exclusive_lock(lock_file_descriptor, lock_file_path);
            return -1;
        }
    ...
    if (file_header.free_offset + total_record_size > file_header.file_size) {
        ...
        if (write_file_header_information(data_file, &file_header) == -1) {
            fclose(data_file);
            release_exclusive_lock(lock_file_descriptor, lock_file_path);
            return -1;
        }
...
record_iterator_t* create_record_iterator(FILE* data_file, lru_cache_t* cache) {
    file_header_t file_header;
    if (read_file_header_information(data_file, &file_header) == -1) return NULL;
    
    record_iterator_t* iterator = malloc(sizeof(record_iterator_t));
...
void free_record_iterator(record_iterator_t* iterator) {
    free(iterator);
}

int read_next_record_from_iterator(record_iterator_t* iterator, record_header_t* record_header, char** json_data) {
    file_header_t file_header;
    if (read_file_header_information(iterator->data_file, &file_header) == -1) return -1;
...
char** find_instances_with_query(const char* database_name, const char* collection_name, const char* query, int* result_count) {
    if (!database_exists(database_name) || !collection_exists(database_name, collection_name)) {
        fprintf(stderr, "Database or collection does not exist\n");
        *result_count = 0;
        return NULL;
    ...
    FILE* data_file = open_data_file_with_optimizations(database_name, collection_name, "rb");
    if (!data_file) {
        *result_count = 0;
        return NULL;
    }
    
    record_iterator_t* iterator = create_record_iterator(data_file, NULL);
    if (!iterator) {
        fclose(data_file);
        *result_count = 0;
        return NULL;
    ...
    while (read_next_record_from_iterator(iterator, &record_header, &json_data) == 1) {
    ...
    iterator = create_record_iterator(data_file, NULL);
    ...
    int current_index = 0;
    while (read_next_record_from_iterator(iterator, &record_header, &json_data) == 1 && current_index < match_count) {
        if (json_matches_query_conditions(json_data, query)) {
...
char** list_all_instances_in_collection(const char* database_name, const char* collection_name, int* instance_count) {
    FILE* data_file = open_data_file_with_optimizations(database_name, collection_name, "rb");
    if (!data_file) {
        *instance_count = 0;
        return NULL;
    ...
    while (read_next_record_from_iterator(iterator, &record_header, &json_data) == 1 && current_index < file_header.record_count) {
...
int parse_insert_data_from_arguments(int argument_count, char* argument_values[], int start_index, 
...
int main(int argument_count, char* argument_values[]) {
    ...
    create_directory_recursively(get_sydb_base_directory_path());
    ...
    if (strcmp(argument_values[1], "create") == 0) {
        ...
        if (argument_count == 3) {
            return create_database(argument_values[2]);
        }
            ...
            if (schema_flag_index != -1) {
                ...
                field_schema_t fields[MAXIMUM_FIELDS];
                int field_count = 0;
                if (parse_schema_fields_from_arguments(argument_count, argument_values, schema_flag_index + 1, 
                ...
                return create_collection(argument_values[2], argument_values[3], fields, field_count);
            }
                ...
                if (parse_insert_data_from_arguments(argument_count, argument_values, insert_flag_index + 1, 
                ...
                char* instance_json = build_instance_json_from_fields_and_values(field_names, field_values, field_count);
                if (!instance_json) {
                ...
                int result = insert_instance_into_collection(argument_values[2], argument_values[3], instance_json);
        ...
        int result_count;
        char** results = find_instances_with_query(argument_values[2], argument_values[3], argument_values[5], &result_count);
        if (result_count > 0) {
        ...
        print_collection_schema(argument_values[2], argument_values[3]);
        return 0;
    }
    else if (strcmp(argument_values[1], "list") == 0) {
        if (argument_count == 2) {
            int database_count;
            char** databases = list_all_databases(&database_count);
            if (database_count == 0) {
            ...
            char** collections = list_collections_in_database(argument_values[2], &collection_count);
            ...
            int instance_count;
            char** instances = list_all_instances_in_collection(argument_values[2], argument_values[3], &instance_count);
            if (instance_count == 0) {
