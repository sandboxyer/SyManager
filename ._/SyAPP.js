...
async displayMenu(menuGeneratorOrObject, configuration = {
  ...
  // Determine initial index based on menu generator (only for function case)
  let initialIndex = 0;
  if (typeof menuGeneratorOrObject === 'function') {
    initialIndex = menuGeneratorOrObject === this.lastMenuGenerator
      ? this.lastSelectedIndex
      : configuration.initialSelectedIndex || 0;
  } else {
    initialIndex = configuration.initialSelectedIndex || 0;
  }
  ...
  // Store reference only for function case
  if (typeof menuGeneratorOrObject === 'function') {
    ...
    this.ProcessFuncs = (FuncClass) => {
      ...
      this.LoadScreen = async (funcname = this.MainFunc.Name,config = {props : {}}) => { 
        if(this.Funcs.has(funcname)){
          ...
          this.displayMenu(return_obj.hud_obj)
        } else {
